import { get_dice_roll_array, get_dice_placeholder_html,get_percentage } from "./utils.js";
...
export class Character {
    constructor(data) {
        ...
        set_dice_html = () => {
            ...
            return this.dice_array_pl_html = this.current_dice_score.map((random_num) => {
                return `<div class="dice">${random_num}</div>`;
            }).join('');
        };

        take_damage = (attack_score_array) => {
            const total_attack_score = attack_score_array.reduce((total_attak, current_attack) => {
        ...
        get_character_html = () => {
            const { element_Id, name, avatar, health, dice_Roll, current_dice_score, dice_Count, dice_array_pl_html } = this;
